...
void BidirectionalCountSketch::process(const void * key1, const void * key2,
...
void BidirectionalCountSketch::update(const void * key1, const void * key2,
		size_t lenKey) {
	auto hashValues = hashFunctionBCS(key1, lenKey);
	for (uint8_t n = 0; n < k_numHashes; n++) {
		uint16_t index = nthHashFunctionBCS(n, hashValues[0], hashValues[1],
...
bool BidirectionalCountSketch::contain(const void * key, size_t lenKey) {
...
uint16_t BidirectionalCountSketch::getMinFrequence(const void * key, size_t lenKey) {
	auto hashValues = hashFunctionBCS(key, lenKey);
...
uint8_t BidirectionalCountSketch::getHashIndex(const void * key1, size_t lenKey,
...
uint32_t BidirectionalCountSketch::getDistNumBCS(const void * key1, size_t lenKey,
		uint32_t key) {
	uint32_t temp = 0;
	auto hashValues = hashFunctionBCS(key1, lenKey);
	for (uint8_t n = 0; n < k_numHashes; n++) {
