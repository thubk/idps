/* MurmurHash - MIT */
#ifndef BIDIRECTIONALCOUNTSKETCH_H_
#define BIDIRECTIONALCOUNTSKETCH_H_
#include <iostream>
#include <string>
#include <vector>
#include <array>
...
class BidirectionalCountSketch {
private:
	uint8_t k_numHashes;
	std::vector<BloomFilter> bf_row;/* bloomfilter */
	std::vector<std::vector<Bucket> > m_bits_row;/*  bucket */
public:
	BidirectionalCountSketch(uint64_t m_bits, uint8_t numHashes) :
...
#endif /* BIDIRECTIONALCOUNTSKETCH_H_ */
